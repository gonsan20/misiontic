...
def calc_sum_diagonal_principal(fila, columna):
    suma = 0
    auxCol = 0
...
def calc_prod_diagonal_secundaria(fila, columna):
    producto = 1
    auxCol = columna - 1
    for i in range(0, fila):
...
def solucion(A):
    ...
    """
    fila = int(str(A.shape)[1:2]) #1-> creo var
    columna = int(str(A.shape)[4:5]) #2-> creo var

    sum_diagonal_principal = calc_sum_diagonal_principal(fila, columna)
    prod_diagonal_secundaria = calc_prod_diagonal_secundaria(fila, columna)
